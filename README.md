...
`study/` — приложение для учёбы: один HTML-файл + PWA-обвязка (manifest, service worker,
иконка). Без зависимостей и сборки, работает офлайн, ставится на телефон как нативное
приложение, всё хранится в localStorage:

- 🤖 **AI-карточки и объяснения** (Claude API, свой ключ — хранится только в браузере):
  кнопка «AI» в колоде генерирует карточки по теме или вставленному конспекту с
  предпросмотром; «Объясни проще» на любой карточке зовёт AI-репетитора
- 🏆 **Лига недели**: глобальный лидерборд на Supabase — ник, XP за неделю, топ-20,
  своя строка подсвечена; XP обнуляется каждый понедельник
- 📊 **График повторений за 30 дней** и онбординг-тур при первом запуске
- 🔔 **UI-звуки** (свист переворота, дзынь/бас ответа, фанфары крита) — отключаются
  в настройках

- ⚔️ **Босс дня**: каждый день новый босс («Прокрастинатор», «Дедлайн», «Билет №13»…) —
  12 вопросов из твоих карточек, 3 жизни, комбо-множитель урона и криты ×2; первая победа
  за день даёт +150 XP, трофеи копятся в статистике
- 🌌 **Вселенная памяти**: живая canvas-карта знаний — каждая карточка это мерцающая звезда,
  чья яркость равна реальной прочности памяти (забытые краснеют, новые — бирюзовые); колоды
  образуют созвездия с подписями и процентом; тап по созвездию — повторение этой колоды.
  Детерминированные позиции звёзд (mulberry32 от id), туманности, до 48 звёзд на колоду
- 🔗 **Колода одной ссылкой**: колода сжимается (deflate) прямо в URL — отправь ссылку
  в мессенджер, у получателя она развернётся в приложении; сервер не нужен
- 🔊 **Озвучка карточек** (TTS) с автоопределением языка — удобно для изучения слов
- 🧠 **Мозг-питомец**: живой персонаж на главной — моргает, спит, радуется и волнуется в
  зависимости от твоего прогресса; растёт с уровнем (очки → шапка выпускника → корона → аура)
  и комментирует происходящее
- ⚡ **XP, уровни и титулы** («Неофит» → «Архимаг знаний») + **3 ежедневных квеста**
  (детерминированные от даты) с прогресс-барами и наградами
- 🃏 **Карточки с интервальным повторением** (SM-2): «Снова / Трудно / Хорошо / Легко»,
  предпросмотр следующего интервала на кнопках, лимит новых карточек в день, клавиши
  `Space` и `1–4`; **cloze-карточки** (пропуски `{{слово}}`, каждый пропуск — отдельная
  карточка) и **режим ввода ответа** с проверкой опечаток (Левенштейн) и подсказкой оценки
- 📉 **Прогноз забывания**: живой процент «прочности памяти» по кривой Эббингауза — на
  карточке, на колоде и общий; 📅 дата экзамена у колоды с обратным отсчётом
- 📋 **Импорт из текста**: вставь строки «термин — определение» (таб / `—` / `-` / `:`) —
  получи колоду; плюс JSON-экспорт/импорт и полный бэкап
- ⚡ **Блиц-квиз** с вариантами ответов из твоих же карточек (cloze тоже участвуют)
- 🍅 **Помодоро** (25/5/15) с автопереходом фокус→пауза (каждая 4-я — длинный отдых) и
  фоновым звуком для концентрации — ливень (розовый шум + капли), прибой (накаты громкости
  и тембра) и тёплый гул-дрон, всё синтезируется в WebAudio без аудиофайлов
- 📈 **Статистика**: стрик и рекорд, точность за 30 дней, теплокарта 12 недель, XP
- 🎨 Тёмная/светлая/авто тема, конфетти, вибрация, всё анимировано
- ☁️ **Облачная синхронизация (Supabase)** — опциональный вход по email/паролю в Настройках;
  local-first: всё работает офлайн, состояние целиком синхронизируется между устройствами
  (last-write-wins по `updatedAt`). SDK грузится с CDN только после первого входа.
  Схема: `database/cortex_setup.sql` (таблица `cortex_state` с RLS, тот же проект, что у трекера)
